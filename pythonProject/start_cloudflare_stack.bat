...
start "BeerMarket API :8091" cmd /k "cd /d "%~dp0" && %PYEXE% -m api.app"

rem --- Start WebApp in dedicated window ---
echo [STEP] Запускаю WebApp: http://localhost:8090
start "BeerMarket WebApp :8090" cmd /k "cd /d "%~dp0webapp" && %PYEXE% -m http.server 8090"
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "try { $r = Invoke-WebRequest -UseBasicParsing http://localhost:8091/health -TimeoutSec 5; if ($r.StatusCode -eq 200) { exit 0 } else { exit 1 } } catch { exit 1 }"
if errorlevel 1 (
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "try { $r = Invoke-WebRequest -UseBasicParsing http://localhost:8090/ -TimeoutSec 5; if ($r.StatusCode -eq 200) { exit 0 } else { exit 1 } } catch { exit 1 }"
if errorlevel 1 (
