...
def _users_kb(token: str, page: int, total: int) -> InlineKeyboardMarkup:
    ...
    users = list(range(start, end))
    for idx in users:
        rows.append([
            InlineKeyboardButton(text=f"👤 #{idx + 1}", callback_data=f"ua:user:{token}:{idx}:{page}")
        ])
...
def _user_actions_kb(token: str, uid: str, page: int, total: int) -> InlineKeyboardMarkup:
    ...
    for idx in range(start, end):
        rows.append([
            InlineKeyboardButton(text=f"#{idx + 1}", callback_data=f"ua:open:{token}:{uid}:{idx}:{page}")
        ])

    nav: list[InlineKeyboardButton] = []
    if page > 0:
        nav.append(InlineKeyboardButton(text="⬅️", callback_data=f"ua:actions:{token}:{uid}:{page - 1}"))
    nav.append(InlineKeyboardButton(text=f"{page + 1}/{last_page + 1}", callback_data="ua:noop"))
    if page < last_page:
        nav.append(InlineKeyboardButton(text="➡️", callback_data=f"ua:actions:{token}:{uid}:{page + 1}"))
    rows.append(nav)
    rows.append([InlineKeyboardButton(text="⬅️ К пользователям", callback_data=f"ua:menu:{token}:0")])
    rows.append([InlineKeyboardButton(text="🔄 Обновить срез", callback_data="ua:refresh")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
def _item_kb(token: str, uid: str, page: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⬅️ К действиям", callback_data=f"ua:actions:{token}:{uid}:{page}")],
...
def _find_uid_by_index(token: str, user_idx: int, deps: UserActivityHandlersDeps) -> str | None:
    users = deps.activity_service.get_users(token)
...
def _user_actions_text(token: str, uid: str, page: int, deps: UserActivityHandlersDeps) -> str | None:
    users = deps.activity_service.get_users(token)
...
def register_user_activity_handlers(router: Router, deps: UserActivityHandlersDeps) -> None:
    ...
    async def activity_user(cq: CallbackQuery):
        if not await deps.ensure_callback_access(cq, "audit.view"):
            return
        parts = (cq.data or "").split(":")
        token = parts[2] if len(parts) > 2 else ""
    ...
    async def activity_user_actions_page(cq: CallbackQuery):
        if not await deps.ensure_callback_access(cq, "audit.view"):
            return
        parts = (cq.data or "").split(":")
        token = parts[2] if len(parts) > 2 else ""
    ...
    async def activity_detail(cq: CallbackQuery):
        ...
        # Совместимость с устаревшим форматом: ua:open:{token}:{user_idx}:{idx}:{page}
        uid = parts[3] if len(parts) > 3 else ""
        ...
        await cq.message.edit_text(row.details, reply_markup=_item_kb(token, uid, page))
        await cq.answer()
